...
description: Evidence-first automation inventory and overlap audit workflow for the toolset. Use when the user wants to know which jobs, hooks, connectors, MCP servers, or wrappers are live, broken, redundant, or missing before fixing anything.
origin: the toolset
---
...
- the user wants to know what was ported from another agent system and what still needs to be rebuilt inside the toolset
...
The value is in collapsing noisy automation into one canonical the toolset lane, not in preserving every historical path.
