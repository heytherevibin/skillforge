...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value the toolset-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
---
...
This is the the toolset-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against the toolset-native coverage
...
- Prefer the toolset-native workflows over generic "install another plugin" advice when the toolset can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in the toolset
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent the toolset workspaces
...
- existing the toolset skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but the toolset lacks a billing-operator skill"
- "Google Drive is connected, but there is no the toolset-native Google Workspace operator workflow"
...
2. whether the toolset already has parity
3. whether the toolset only has primitives
4. whether the toolset is missing the workflow entirely

### Phase 3: Turn Gaps Into the toolset Decisions

For every real gap, recommend the correct the toolset-native shape:

| Gap Type | Preferred the toolset Shape |
|----------|---------------------|
...
2. **Parity**
   - where the toolset already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but the toolset lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete the toolset-native additions, ordered by impact
...
- If the toolset already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what the toolset should own next.
